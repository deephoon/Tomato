...
## 13. 향후 로드맵 (v2.0 업데이트 계획)

현재 분석을 바탕으로 2026년 최신 웹 UI/UX 트렌드를 반영한 다음 단계 업데이트 계획을 수립했습니다.

1. **보안 및 환경 변수 통합 (.env)**
   - UI에서 직접 입력받던 Gemini API Key를 `.env` 파일의 `VITE_GEMINI_API_KEY`로 통합 관리하여 로컬 스토리지 노출을 방지합니다.
2. **데스크탑 알림 탑재 (Notification API)**
   - 앱이 백그라운드에 있거나 최소화되어 있을 때도 타이머 종료 시 즉시 알림을 받을 수 있도록 네이티브 브라우저 시스템 알림을 추가합니다.
3. **2026 아트 트렌드를 반영한 감각적 비주얼 리뉴얼**
   - **Tactile Maximalism & Glassmorphism 2.0**: 단순 평면을 넘어 물성이 느껴지는 텍스처와 네온 글로우(Neon Glow) 효과를 더해, 힙하고 트렌디한 깊이감을 부여합니다.
   - **Lightweight 3D 최적화**: 화려하고 감각적인 인터랙션을 유지하면서도 폴리곤 및 디바이스 렌더링 픽셀 해상도를 맞춤형으로 튜닝해 앱 퍼포먼스를 가볍게 유지합니다.
   - **타이포그래피 및 레이아웃**: 시대를 앞서가는 현대적 폰트와 여백을 도입하여 UI의 완성도를 극대화합니다.
